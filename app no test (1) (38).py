...
# حالات قسم كرر
user_repeat_type = {}  # تتبع نوع كرر لكل مستخدم: "حرفين" / "ثلاث" / "اربع" / "خمس" / "مختلط" أو None (افتراضي)
...
# نظام الموافقة على النقاط العالية: {uid_{typ}_{device_type}: {uid, typ, wpm, device_type, username}}
pending_high_speed_scores = {}

# حالة إضافة أعلى الدزات للمالك الأساسي
add_top_speed_state = {}

# وضع مقياس السرعة لكل مستخدم: "bot" (افتراضي) أو "monkeytime"
user_scale_mode = {}
...
# كلمات كرر حسب نوع التكرار
REPEAT_TWO_LETTERS = ["سم", "يد", "ام", "اي", "جد", "سن", "حب", "كم", "حي", "حق", "حس", "خط", "ظل", "بر", "قم", "شد", "بس", "صب", "تم", "دب", "صف", "حد", "لا", "ما", "في", "من", "لو", "دم", "هي", "كل", "له", "لي", "مت", "ذا", "هو", "مع", "او", "لب", "صك", "كف", "ود"]

REPEAT_THREE_LETTERS = ["كلب", "شمس", "وصل", "بحر", "قلم", "برق", "ريح", "صقر", "سمك", "جبل", "حلم", "ملك", "باب", "صوت", "رسم", "طير", "عسل", "بنت", "ضحك", "خبر", "ملح", "جبن", "علم", "وين", "كيف", "سور", "جسم", "مشي", "ايه", "نفس", "جرس", "مسك", "فيك"]

REPEAT_FOUR_LETTERS = ["طريق", "قلبك", "حبيب", "جمله", "مقال", "عميل", "جديد", "وينه", "محمد", "خصيم", "نادر", "شموخ", "رجال", "عليك", "كثير", "شروق", "ايام", "ايوه", "كتاب", "عروض", "طيار", "قريب", "جميل", "خبير", "حياة", "سريع", "جرير"]

REPEAT_FIVE_LETTERS = ["خلاص", "عاديه", "كتابه", "كثيره", "شمالي", "جنوبي", "خساره", "جريده", "مشكله", "صغيره", "كلمات", "خميره", "شهامه", "مرحبا", "مليون", "دولار", "يسعدك", "باردة", "مواقع", "سعودي", "اتحاد", "أصناف", "صداره", "صلابه", "كابوس", "احلام", "الناس", "اختراع", "الظلم", "ظلمات", "دقايق"]
...
# تتبع حالة إدارة الصدارة (تحميل/تثبيت): {uid: {stage: waiting_mobile/waiting_external, files: {}}}
leaderboard_management_state = {}
...
def generate_very_easy_numbers_sentence(count=None):
    # 0-10 فقط
    if count is None:
        count = random.randint(5, 10)
    else:
...
def normalize_number_text(text):
    ...
    replacements = {
        ...
        r'\b(صفر|زيرو|zero)\b': 'صفر',
        r'\b(واحد|احد|ون)\b': 'واحد',
        r'\b(اثنين|اثنان|تو)\b': 'اثنين',
        r'\b(ثلاث|ثلاثه|ثري)\b': 'ثلاثه',
        r'\b(اربع|اربعه|فور)\b': 'اربعه',
        r'\b(خمس|خمسه|فايف)\b': 'خمسه',
        r'\b(سته|ست|سكس)\b': 'سته',
        r'\b(سبعه|سبع|سفن)\b': 'سبعه',
        r'\b(ثمانيه|ثمان|ثمن|ايت)\b': 'ثمانيه',
        r'\b(تسعه|تسع|ناين)\b': 'تسعه',
        r'\b(عشرة|عشره|تن|عشر)\b': 'عشره',
...
def get_speed_message(username, wpm, elapsed_time, neutral=False):
    """اختيار رسالة عشوائية بناءً على السرعة"""
    if not neutral and wpm > 200:
        # رسائل تشجيعية للسرعة العالية (أكثر من 200)
        messages = [
            f"اطربني يا {username} ذي الأرقام الصح\n\nسرعتك: {wpm:.2f} كلمة/دقيقة\nالوقت : {elapsed_time:.2f} ثانية",
            f"رقم اسطوري يا {username} اهنيك\n\nسرعتك: {wpm:.2f} كلمة/دقيقة\nالوقت : {elapsed_time:.2f} ثانية",
            f"يا جنونك سرعة تاريخية ودزة عالميه\n\nسرعتك: {wpm:.2f} كلمة/دقيقة\nالوقت : {elapsed_time:.2f} ثانية"
        ]
        return random.choice(messages)
    elif not neutral and wpm <= 50:
        # رسائل سخرية للسرعة المنخفضة (50 وأقل)
        messages = [
            f"يخوي تكتب برجولك انت؟\n\nسرعتك: {wpm:.2f} كلمة/دقيقة\nالوقت : {elapsed_time:.2f} ثانية",
            f"اكتب يا {username} زين وش الرقم الخايس ذا\n\nسرعتك: {wpm:.2f} كلمة/دقيقة\nالوقت : {elapsed_time:.2f} ثانية",
            f"يخوي سرعتك تجيب الهم\n\nسرعتك: {wpm:.2f} كلمة/دقيقة\nالوقت : {elapsed_time:.2f} ثانية"
        ]
        return random.choice(messages)
    else:
        # الرسالة العادية للسرعات العادية
        return f"كفو يا {username}\n\nسرعتك: {wpm:.2f} كلمة/دقيقة\nالوقت : {elapsed_time:.2f} ثانية"
...
async def speed_bot_type_sentence(context, cid, sentence, wpm, sentence_type, start_time):
    try:
        ...
        final_text = get_speed_message("مستخدم", actual_wpm, elapsed_time)
        try:
            if message:
...
class Storage:
    ...
    def is_chat_paused(self, cid):
        cid_str = str(cid)
        if cid_str not in self.data.get("chats", {}):
            return False
        return self.data["chats"][cid_str].get("paused", False)

    def set_chat_paused(self, cid, paused):
        cid_str = str(cid)
        if cid_str not in self.data.get("chats", {}):
            self.data["chats"][cid_str] = {
                "title": "",
                "created_at": datetime.now().isoformat(),
                "paused": paused
            }
        else:
            self.data["chats"][cid_str]["paused"] = paused
        self.mark_dirty()
    ...
    def is_main_owner(self, uid):
        # فقط المالك الرئيسي الرئيسي (الـ OWNER_ID) هو المالك الأساسي
        return uid == OWNER_ID
    ...
    def update_score(self, uid, typ, wpm, device_type=None, accuracy=None):
        # لا تحفظ أي رقم في الصدارة إلا إذا كانت الدقة 100% بالضبط
        if accuracy is not None and accuracy != 100.0:
            print(f"[SCORE] User {uid} - Section {typ} - WPM: {wpm:.1f} - NOT SAVED (accuracy {accuracy:.1f} != 100)")
            return False
        ...
        # إذا كان القسم كرر وكان النوع "حرفين"، لا تحفظ النقاط
        if typ == "كرر":
            uid_repeat_type = user_repeat_type.get(uid)
            if uid_repeat_type == "حرفين":
                print(f"[SCORE] User {uid} - Section {typ} - WPM: {wpm:.1f} - NOT SAVED (using كرر حرفين)")
                return False
        ...
        # إذا لم يحدد المستخدم نوع جهاز أصلاً، لا تحفظ النقاط
        if device_type is None:
            print(f"[SCORE] User {uid} - Section {typ} - WPM: {wpm:.1f} - NOT SAVED (device_type not set)")
            return False

        # طبقة حماية صدارة الجوال:
        # - لقسم 'جب' الحد 210 WPM
        # - لباقي الأقسام الحد 190 WPM
        mobile_threshold = 210 if typ == "جب" else 190
        is_high_speed_mobile = (device_type == "جوال" and wpm >= mobile_threshold and typ != "كرر")
        
        if "flagged_high_speed_mobile" not in self.data:
            self.data["flagged_high_speed_mobile"] = {}
        
        key = f"{uid}_{typ}_{device_type}"
        old_score = self.leaderboard_data["scores"].get(key, 0)

        # السرعات العالية على الجوال تُعلّق للمراجعة ولا تُحفظ مباشرة
        if is_high_speed_mobile and wpm > old_score:
            self.data["flagged_high_speed_mobile"][key] = True
            print(f"[HIGH_SPEED_MOBILE] User {uid} - Section {typ} - WPM: {wpm:.1f} - PENDING OWNER APPROVAL")
            self.mark_dirty()
            return True
        ...
        # أزل الراية إن كانت النقطة الحالية طبيعية أو خارجي
        if not is_high_speed_mobile:
            self.data["flagged_high_speed_mobile"].pop(key, None)
        ...
        self.mark_dirty()
        # إرجاع True إذا تم فلترة النقطة بسبب سرعة عالية
        return is_high_speed_mobile
    ...
    def get_leaderboard(self, typ, device_type=None):
        ...
        flagged_high_speed = self.data.get("flagged_high_speed_mobile", {})
        ...
        for k, v in self.leaderboard_data["scores"].items():
            ...
            # فلتر صدارة الجوال: استبعد الأرقام المفلترة (>= 190 WPM على جوال)
            if device_type == "جوال" and k in flagged_high_speed:
                continue
    ...
    def clear_leaderboard_scores(self, uid, keep_backup=False, clear_removed_flag=False):
        """حذف جميع أرقام المستخدم من الصدارة.

        keep_backup: حفظ نسخة احتياطية للأرقام الحالية.
        clear_removed_flag: فك المنع من الظهور في الصدارة إن وُجد.
        """
        if self.is_main_owner(uid):
            print(f"[LEADERBOARD] Cannot clear scores for primary owner {uid}")
            return []
        ...
        scores = self.leaderboard_data.setdefault("scores", {})
        flagged_high_speed = self.data.setdefault("flagged_high_speed_mobile", {})
        low_word_count = self.data.setdefault("low_word_count_scores", {})
        removed_list = self.leaderboard_data.setdefault("removed_from_leaderboard", {})
        ...
        for k in list(scores.keys()):
            ...
            if keep_backup:
                backup_data[k] = scores[k]
            keys_to_remove.append(k)

        if keep_backup and backup_data:
            self.data["leaderboard_backups"][uid_str] = backup_data
        ...
        for k in keys_to_remove:
            scores.pop(k, None)
            flagged_high_speed.pop(k, None)
            low_word_count.pop(k, None)

        if clear_removed_flag:
            removed_list.pop(uid_str, None)

        if keys_to_remove or clear_removed_flag:
            self.save_leaderboard()
            self.mark_dirty()
            self.save(force=True)
    ...
    def transfer_leaderboard_scores(self, uid, target_device_type):
        """نقل أرقام المستخدم من أي صدارة أخرى إلى صدارة الجهاز المطلوب."""
        if self.is_main_owner(uid):
            print(f"[LEADERBOARD] Cannot transfer scores for primary owner {uid}")
            return []

        if target_device_type not in ["جوال", "خارجي"]:
            return []
        ...
        for k, v in list(scores.items()):
            ...
            if parts[-1] in ["جوال", "خارجي"]:
                ...
                key_device_type = self.get_device_type(uid) or "جوال"
                section_type = '_'.join(parts[1:])

            if not section_type:
                continue

            if key_device_type == target_device_type:
                continue

            new_key = f"{uid}_{section_type}_{target_device_type}"
            updates[new_key] = max(v, scores.get(new_key, 0), updates.get(new_key, 0))
            keys_to_remove.append(k)
            moved_sections.add(section_type)
        ...
        for k, v in updates.items():
            scores[k] = v
            if k.endswith("_خارجي"):
                flagged_high_speed.pop(k, None)

        self.save_device_type(uid, target_device_type)

        if keys_to_remove or updates:
            self.save_leaderboard()
            self.mark_dirty()
            self.save(force=True)

        return list(moved_sections)

    def restore_from_leaderboard(self, uid, restore_scores=False, clear_scores=False):
        ...
        if uid_str in self.leaderboard_data["removed_from_leaderboard"]:
            ...
            if restore_scores:
                ...
                self.clear_leaderboard_scores(uid, keep_backup=False, clear_removed_flag=False)
    ...
    def remove_section_scores(self, uid, section, device_type):
        ...
        keys_to_remove = []
        for k in list(self.leaderboard_data["scores"].keys()):
            if not k.startswith(f"{uid_str}_"):
                continue
    ...
    def add_top_speed(self, username, speed, section, message_id):
        if "top_speeds" not in self.data:
            self.data["top_speeds"] = []
        self.data["top_speeds"].append({
            "username": username,
            "speed": speed,
            "section": section,
    ...
    def get_top_speeds(self):
        return self.data.get("top_speeds", [])
...
class RandomURLManager:
    """مدير يختار رابط عشوائي من قائمة روابط ويحمل 20000 جملة منه"""
    def __init__(self, section_name, urls_list, min_words=5, max_words=50, lang="arabic", max_sentences=20000):
        self.section_name = section_name
    ...
    def load(self):
        try:
            # اختر رابط عشوائي من القائمة
            self.current_url = random.choice(self.urls_list)
            print(f"[{self.section_name}] جاري تحميل البيانات من رابط عشوائي...")
            
            direct_url = self._convert_gdrive_url_to_direct(self.current_url)
            response = requests.get(direct_url, timeout=30)

            if response.status_code != 200:
                print(f"[{self.section_name}] فشل تحميل الرابط: HTTP {response.status_code}")
                return
            ...
            for line in lines:
                ...
                if self.min_words <= word_count <= self.max_words:
                    valid_lines.append(cleaned_line)
                    if len(valid_lines) >= self.max_sentences:
                        break
            
            self.sentences = valid_lines[:self.max_sentences]
            self.last_update = time.time()
            print(f"[{self.section_name}] تم تحميل {len(self.sentences)} جملة من الرابط العشوائي بنجاح")
...
class MultiURLJSONManager:
    """مدير يدعم عدة روابط ويحمل البيانات في الرام"""
    def __init__(self, section_name, urls, min_words=5, max_words=50, lang="arabic", max_urls=10, max_sentences_per_url=3000):
        self.section_name = section_name
        self.urls = urls[:max_urls]  # أخذ أول max_urls روابط
        self.min_words = min_words
        self.max_words = max_words
        self.lang = lang
        self.max_sentences_per_url = max_sentences_per_url
        self.sentences = []
        self.last_update = 0
    ...
    def load(self):
        try:
            print(f"[{self.section_name}] جاري تحميل البيانات من {len(self.urls)} رابط...")
            all_sentences = []
            ...
            for url in self.urls:
                try:
                    ...
                    if response.status_code != 200:
                        print(f"[{self.section_name}] فشل تحميل {url}: HTTP {response.status_code}")
                        continue
                    ...
                    for line in lines:
                        ...
                        word_count = len(clean_text_for_word_count(cleaned_line).split())
                        if self.min_words <= word_count <= self.max_words:
                            url_sentences.append(cleaned_line)
                            if len(url_sentences) >= self.max_sentences_per_url:
                                break
                    
                    # أضف الجمل من هذا الرابط (محدودة بالحد الأقصى)
                    all_sentences.extend(url_sentences[:self.max_sentences_per_url])
                            
                except Exception as e:
                    print(f"[{self.section_name}] خطأ في تحميل {url}: {e}")
                    continue

            self.sentences = all_sentences
            self.last_update = time.time()
            print(f"[{self.section_name}] تم تحميل {len(self.sentences)} جملة من {len(self.urls)} رابط")
...
def count_wrong_chars(orig, usr, lang="arabic"):
    """عدد الأحرف الخاطئة (غير المتطابقة) بين النص الأصلي والمكتوب."""
    try:
        if lang == "persian":
            o = normalize_persian(orig)
            u = normalize_persian(usr)
        ...
        return max(0, max(len(o), len(u)) - matched)
    except Exception:
        return 0
...
def gen_pattern(uid, count=1, exclude_words=None):
    ...
    # حديد نوع كرر بناءً على اختيار المستخدم
    repeat_type = user_repeat_type.get(uid, None)
    
    if repeat_type == "حرفين":
        words_list = REPEAT_TWO_LETTERS
    elif repeat_type == "ثلاث":
        words_list = REPEAT_THREE_LETTERS
    elif repeat_type == "اربع":
        words_list = REPEAT_FOUR_LETTERS
    elif repeat_type == "خمس":
        words_list = REPEAT_FIVE_LETTERS
    elif repeat_type == "مختلط":
        # اختر من كل قسم عشوائياً
        all_lists = [REPEAT_TWO_LETTERS, REPEAT_THREE_LETTERS, REPEAT_FOUR_LETTERS, REPEAT_FIVE_LETTERS]
        words_list = []
        for _ in range(count):
            words_list.append(random.choice(random.choice(all_lists)))
        # إذا كان مختلط، أرجع النمط مباشرة مع الأرقام
        pattern = []
        ...
        # افتراضي: استخدم الكلمات الأصلية
        words_list = REPEAT_WORDS

    pattern = []
    used_words_in_pattern = []
    available_words = [w for w in words_list if w not in exclude_words]
...
def resolve_target_user(target):
    """تحويل الأيدي أو اليوزر إلى معرف واسم عرض."""
    if not target:
    ...
    if cleaned.isdigit():
        target_uid = int(cleaned)
    else:
        target_uid = get_user_id_by_username(cleaned)
    ...
    username = user_data.get("username")
    first_name = user_data.get("first_name") or user_data.get("name") or cleaned
    display_name = f"@{username}" if username else first_name
    return target_uid, user_data, display_name

async def bot_can_kick_members(bot, chat_id):
    """التحقق أن البوت يملك صلاحية طرد الأعضاء في مجموعة معينة."""
    try:
        me = await bot.get_me()
        me_member = await bot.get_chat_member(chat_id, me.id)
        if me_member.status == "creator":
            return True
        if me_member.status == "administrator":
            return bool(getattr(me_member, "can_restrict_members", False))
        return False
    except Exception:
        return False

async def kick_user_from_chat(bot, chat_id, target_uid):
    """طرد مستخدم من مجموعة إذا كان قابلاً للطرد."""
    try:
    ...
    try:
        ...
        await bot.unban_chat_member(chat_id=chat_id, user_id=target_uid, only_if_banned=False)
        return True, "kicked"
    except Exception:
        return False, "failed"
...
async def check_and_ban_cheater(
    ...
    """Check if user is cheating (WPM >= 300 with 100% accuracy in non-repeat sections) and ban them
...
managers = {
    ...
    "قص": LocalJSONManager("قص", URLS["قص"], min_words=5, max_words=50),
    "نص": RandomURLManager("نص", NASS_DRIVE_URLS, min_words=5, max_words=50, max_sentences=20000),
...
async def show_bot_commands(u: Update, c: ContextTypes.DEFAULT_TYPE, is_callback=False):
    ...
    msg = (
        ...
        "-\n"
        "(تحويل المقياس) يبدل لك بين مقياس البوت ومونكي تايب (فيه RAW) \n"
        "-----------------------------\n"
        "- (أون لاين) - لعب مباريات 1v1\n"
        "- (سجلي) - عرض إحصائيات أون لاين\n"
        "-----------------------------\n"
        "- (الصدارة) - المتصدرين\n"
        "- (أعلى الدزات) افضل دزات في البوت \n"
        ...
        "-----------------------------\n"
        "(جب او كرر حرفين) يخلي القسم ينزل كلمات من حرفين \n"
        "(جب او كرر ثلاث حروف) يخلي القسم ينزل كلمات من ثلاث حروف\n"
        "(جب او كرر اربع حروف) يخلي القسم ينزل كلمات من أربع حروف \n"
        "(جب او كرر خمسه حروف) يخلي القسم ينزل كلمات من خمس حروف \n"
        "(جب او كرر مختلط) من كل الأقسام كلمات \n"
        "(جب او كرر قديم) نفس كلمات البوت الافتراضية (الاساسيه) \n"
        "-----------------------------\n"
...
async def show_all_bot_commands(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    msg = (
        "دليل البوت الكامل - شرح كل أمر حرفياً\n"
        "-----------------------------------\n\n"
        "هذا الدليل يشرح: اسم الأمر + وظيفته + طريقة كتابته.\n"
        "تنبيه: بعض الأوامر مقيدة بالصلاحيات.\n\n"

        "[أولاً] أوامر الواجهة والمساعدة\n"
        "1) عرض / بوت / مقالات\n"
        "- ماذا يفعل: يفتح لك رسالة أقسام البوت (نفس واجهة البداية).\n"
        "- الاستخدام: اكتب أي كلمة من الثلاثة.\n\n"

        "2) عرض جميع الاوامر / جميع الاوامر / الاوامر كاملة\n"
        "- ماذا يفعل: يرسل لك هذا الدليل الكامل.\n"
        "- الاستخدام: اكتب أي صيغة منها.\n\n"

        "3) إدارة / اداره / ادارة\n"
        "- ماذا يفعل: يرسل لوحة أوامر الإدارة المختصرة.\n"
        "- الاستخدام: اكتب إدارة.\n\n"

        "[ثانياً] أوامر اللعب بالأقسام\n"
        "الأقسام الأساسية: عشوائي، نص، جمم، ويكي، صج، جب، شك، جش، قص، كرر، شرط، فكك، دبل، تر، عكس، فر، E، رق، حر، خصص.\n"
        "- ماذا تفعل: كل قسم يرسل لك جملة/كلمات من نوعه لتكتبها.\n"
        "- الاستخدام: اكتب اسم القسم مباشرة (مثال: ويكي).\n\n"

        "طلب عدد كلمات من قسم:\n"
        "- ماذا يفعل: يرسل نص من نفس القسم بعدد كلمات محدد.\n"
        "- الاستخدام: [اسم القسم] [العدد]\n"
        "- مثال: جمم 40 / ويكي 25.\n\n"

        "كشف + غير الكشف:\n"
        "- ماذا يفعل: يفعّل قسم كشف الغش، و(غير الكشف) يغيّر طريقة الكشف.\n"
        "- الاستخدام: كشف ، ثم غير الكشف للتبديل.\n\n"

        "رق + غير الرق + ريست رق:\n"
        "- ماذا يفعل: قسم الأرقام، وتغيير نظام عرض الأرقام، ثم إعادته الافتراضي.\n"
        "- الاستخدام: رق / غير الرق / ريست رق.\n\n"

        "ريست + ريست [قسم]:\n"
        "- ماذا يفعل: ريست يعيد كل الإعدادات الافتراضية، وريست قسم يعيد قسم واحد فقط.\n"
        "- الاستخدام: ريست أو ريست كرر (مثال).\n\n"

        "[ثالثاً] أوامر التخصيص\n"
        "1) خصص\n"
        "- ماذا يفعل: يبدأ معك خطوات إنشاء تخصيص كلمات خاص بك.\n"
        "- الاستخدام: اكتب خصص واتبع التعليمات.\n\n"

        "2) المحفوظ\n"
        "- ماذا يفعل: يعرض التخصيصات المحفوظة لديك.\n"
        "- الاستخدام: المحفوظ\n\n"

        "3) حذف تخصيص [اختصار]\n"
        "- ماذا يفعل: يحذف تخصيص معين بالاختصار.\n"
        "- مثال: حذف تخصيص a1\n\n"

        "4) كلمات تخصيص [اختصار]\n"
        "- ماذا يفعل: يعرض كلمات التخصيص نفسه.\n"
        "- مثال: كلمات تخصيص a1\n\n"

        "5) تعديل تخصيص [اختصار]\n"
        "- ماذا يفعل: يفتح تعديل على نفس التخصيص.\n"
        "- مثال: تعديل تخصيص a1\n\n"

        "[رابعاً] أوامر جب/كرر حسب عدد الحروف\n"
        "جب: جب حرفين / جب ثلاث حروف / جب اربع حروف / جب خمسه حروف / جب مختلط / جب قديم\n"
        "كرر: كرر حرفين / كرر ثلاث حروف / كرر اربع حروف / كرر خمس حروف / كرر مختلط / كرر قديم\n"
        "الصيغة المشتركة: جب او كرر حرفين/ثلاث/اربع/خمسه/مختلط/قديم\n"
        "- ماذا تفعل: تغيّر نوع الكلمات التي ينزلها قسم جب أو كرر.\n\n"

        "[خامساً] الدقة والمقياس والسرعة\n"
        "1) تفعيل الدقة\n"
        "- ماذا يفعل: يحتسب دقة رسائلك حتى مع أخطاء بسيطة.\n"
        "2) تعطيل الدقة\n"
        "- ماذا يفعل: يرجع التقييم للوضع الصارم.\n"
        "3) تحويل المقياس\n"
        "- ماذا يفعل: يبدّل حساب السرعة بين مقياس البوت ومقياس Monkeytype (RAW).\n"
        "4) سبيد\n"
        "- ماذا يفعل: يعرض سرعة كتابتك الحالية.\n"
        "5) سبيد وقف\n"
        "- ماذا يفعل: يوقف متابعة السرعة.\n"
        "6) تلقائي\n"
        "- ماذا يفعل: يشغل وضع نزول الجمل بشكل متتابع تلقائياً.\n\n"

        "[سادساً] أوامر أون لاين 1v1\n"
        "1) أون لاين / اون لاين\n"
        "- ماذا يفعل: يدخل البحث عن خصم ويبدأ مباراة 1v1.\n"
        "2) سجلي\n"
        "- ماذا يفعل: يعرض إحصائياتك في الأون لاين (فوز/خسارة/انسحاب).\n"
        "3) انسحب\n"
        "- ماذا يفعل: تستسلم من المباراة الحالية.\n"
        "4) شطب\n"
        "- ماذا يفعل: طلب تخطي الجملة الحالية (بموافقة الخصم).\n"
        "5) الغاء\n"
        "- ماذا يفعل: يلغي البحث عن خصم إذا كنت في الانتظار.\n"
        "6) صدارة الاون لاين\n"
        "- ماذا يفعل: يعرض ترتيب أكثر اللاعبين انتصاراً.\n\n"

        "[سابعاً] أوامر الجولات داخل القروبات\n"
        "1) فتح جولة / فتح جوله\n"
        "- ماذا يفعل: يبدأ جولة جديدة في القروب.\n"
        "2) جولة\n"
        "- ماذا يفعل: يعرض حالة الجولة الحالية وإحصائياتها.\n"
        "3) قفل جولة / قفل جوله\n"
        "- ماذا يفعل: ينهي الجولة الجارية.\n"
        "4) مدد\n"
        "- ماذا يفعل: يمدد الجولة بعد نهايتها.\n"
        "5) منظم\n"
        "- ماذا يفعل: يعرض من هو منظم الجولة.\n"
        "6) ة\n"
        "- ماذا يفعل: يمسح محاولتك الأخيرة.\n"
        "7) ق\n"
        "- ماذا يفعل: يلغي الجولة.\n"
        "8) قف\n"
        "- ماذا يفعل: يوقف متابعة الكتابة للجولة.\n"
        "9) شيل\n"
        "- ماذا يفعل: يزيل آخر رسالة/محاولة مرتبطة باللعب.\n\n"

        "[ثامناً] الصدارة، الجوائز، والتقدم\n"
        "1) الصدارة / توب\n"
        "- ماذا يفعل: يعرض صدارة الأقسام حسب نوع جهازك.\n"
        "2) جوائزي\n"
        "- ماذا يفعل: يعرض الجوائز اللي حصلت عليها.\n"
        "3) تقدمي\n"
        "- ماذا يفعل: يعرض المستوى الحالي ونسبة التقدم.\n"
        "4) المزالين\n"
        "- ماذا يفعل: يعرض قائمة من تم إزالتهم من الصدارة.\n"
        "5) أعلى الدزات / اعلى الدزات / توب الدزات\n"
        "- ماذا يفعل: يعرض أفضل الدزات المثبتة في البوت.\n\n"

        "[تاسعاً] إدارة الصدارة (بالأيدي أو اليوزر)\n"
        "1) ازل [ID/@user]\n"
        "- ماذا يفعل: يحذف المستخدم من الصدارة ويمنعه من الظهور مستقبلاً.\n"
        "- الرجوع يكون فقط بأمر رجع أو ارجع.\n\n"

        "2) ازل بدون حظر [ID/@user]\n"
        "- ماذا يفعل: يحذف أرقامه الحالية فقط بدون منعه.\n"
        "- أي رقم جديد يجيبه لاحقاً يدخل الصدارة مباشرة.\n\n"

        "3) ارجع [ID/@user]\n"
        "- ماذا يفعل: يرجعه للصدارة بدون استرجاع أرقامه القديمة.\n\n"

        "4) رجع [ID/@user]\n"
        "- ماذا يفعل: يفتح لك خيارين (استعادة مع الأرقام السابقة / بدون أرقام).\n\n"

        "5) حذف [ID/@user] من صدارة جوال قسم [القسم]\n"
        "6) حذف [ID/@user] من صدارة خارجي قسم [القسم]\n"
        "- ماذا يفعل: يحذف رقم هذا المستخدم من قسم محدد فقط.\n\n"

        "7) حول ارقام [ID/@user] لخارجي\n"
        "- ماذا يفعل: ينقل أرقامه من صدارة الجوال إلى صدارة الخارجي ويعيد ترتيبه حسب كل قسم.\n\n"

        "8) اضف [ID/@user] لصدارة [جوال/خارجي] قسم [القسم] بسرعة [الرقم]\n"
        "- ماذا يفعل: إضافة رقم يدوي مباشر لمستخدم في قسم محدد.\n\n"

        "9) تحميل صدارة\n"
        "- ماذا يفعل: يفتح خيارات تحميل/حفظ ملفات الصدارة.\n\n"

        "10) ريست صدارة\n"
        "- ماذا يفعل: يصفر الصدارة ويوزع الجوائز حسب النظام.\n\n"

        "[عاشراً] الباند والإشراف\n"
        "1) باند / حظر\n"
        "- ماذا يفعل: يحظر المستخدم من البوت.\n"
        "2) الغاء باند / فك باند / الغاء حظر\n"
        "- ماذا يفعل: يفك الحظر.\n"
        "3) باند سريع\n"
        "- ماذا يفعل: يدخل وضع باند سريع للمالك الأساسي.\n"
        "4) رفع ادمن / رفع مالك\n"
        "- ماذا يفعل: ترقية رتب الإشراف.\n"
        "5) إزالة ادمن / ازاله ادمن / إزالة مالك / ازالة مالك\n"
        "- ماذا يفعل: سحب الرتبة.\n"
        "6) تنزيل ادمن / تنزيل أدمن / تنزيل مالك / نزل مالك\n"
        "- ماذا يفعل: تنزيل الرتبة لعضو.\n"
        "7) ترقية ادمن [ID/@user] / ترقية مالك [ID/@user]\n"
        "- ماذا يفعل: ترقية مباشرة بدون رد على رسالة.\n"
        "8) تنزيل ادمن [ID/@user] / تنزيل مالك [ID/@user]\n"
        "- ماذا يفعل: تنزيل مباشر بدون رد.\n"
        "9) حذف [ID] من الإشراف\n"
        "- ماذا يفعل: يزيل المستخدم من قوائم الملاك/الأدمن.\n"
        "10) أيدي الإشراف / ايدي الاشراف\n"
        "- ماذا يفعل: يعرض قائمة المشرفين والملاك.\n"
        "11) ريست ادمن / حذف ادمن\n"
        "- ماذا يفعل: حذف جميع الأدمنية.\n"
        "12) ريست المالك / ريست ملاك / حذف المالك / حذف ملاك\n"
        "- ماذا يفعل: حذف جميع الملاك.\n\n"

        "[أحد عشر] الإذاعة، الإحصاء، النظام\n"
        "1) إذاعة\n"
        "- ماذا يفعل: يبدأ إرسال إذاعة عامة للمستخدمين.\n"
        "2) اذاعة الخاص\n"
        "- ماذا يفعل: إذاعة للمحادثات الخاصة فقط.\n"
        "3) اذاعة القروبات\n"
        "- ماذا يفعل: إذاعة للمجموعات فقط.\n"
        "4) احصاء\n"
        "- ماذا يفعل: إحصائيات البوت العامة (مستخدمين/قروبات/محظورين/أقسام).\n"
        "5) تفاعل البوت\n"
        "- ماذا يفعل: يعرض عدد الأوامر اليوم وأمس.\n"
        "6) المستخدمين\n"
        "- ماذا يفعل: عدد المستخدمين الإجمالي.\n"
        "7) عرض الكل\n"
        "- ماذا يفعل: قائمة جميع المستخدمين المسجلين.\n"
        "8) المحظورين\n"
        "- ماذا يفعل: قائمة المحظورين.\n"
        "9) روابط القروبات\n"
        "- ماذا يفعل: روابط/مراجع القروبات المسجلة.\n"
        "10) وقف هنا / شغل هنا\n"
        "- ماذا يفعل: إيقاف أو تشغيل البوت في القروب الحالي.\n"
        "11) صنفي\n"
        "- ماذا يفعل: يعرض تصنيف جهازك الحالي (جوال/خارجي).\n"
        "12) صنف\n"
        "- ماذا يفعل: تغيير الصنف (لمن لديه الصلاحية).\n"
        "13) تغيير جوال [ID/@user] / تغيير خارجي [ID/@user]\n"
        "- ماذا يفعل: تعديل تصنيف جهاز مستخدم.\n"
        "14) تسجيل جوال [ID/@user] / تسجيل خارجي [ID/@user]\n"
        "- ماذا يفعل: تسجيل نوع جهاز مستخدم جديد.\n"
        "15) ايديه\n"
        "- ماذا يفعل: يطلع ID المستخدم (غالباً بالرد).\n"
        "16) سبام متواصل / قفل سبام\n"
        "- ماذا يفعل: تشغيل/إيقاف سبام متواصل في القروب.\n"
        "17) اضافه إلى توب الدزات\n"
        "- ماذا يفعل: يضيف دزة يدوية لقائمة أعلى الدزات بخطوات تفاعلية.\n\n"

        "[إثنا عشر] أوامر الطرد الجماعي والموجه\n"
        "1) طرد الكل\n"
        "- ماذا يفعل: يطرد كل المستخدمين القابلين للطرد في المجموعة الحالية.\n"
        "- ملاحظة: يحتاج صلاحية طرد عند البوت، ويعمل للمالك الأساسي فقط.\n\n"

        "2) طرد كل اللي في [أيدي المجموعة]\n"
        "- ماذا يفعل: ينفذ طرد جماعي في المجموعة المحددة بالأيدي، حتى لو المنفذ مو موجود داخلها.\n"
        "- ملاحظة: يحتاج صلاحية طرد عند البوت في تلك المجموعة.\n\n"

        "3) طرد [@يوزر أو أيدي] من مجموعة [أيدي المجموعة]\n"
        "- ماذا يفعل: يطرد مستخدم محدد من مجموعة محددة.\n"
        "- مثال: طرد @username من مجموعة -1001234567890\n\n"

        "إذا تبغى نسخة ثانية أدق: (أمر + الصلاحية المطلوبة + أمثلة جاهزة نسخ/لصق)، أقدر أضيفها لك بنفس الرسالة."
    )

    # تجزئة الرسالة حتى لا تتجاوز حد تيليجرام
    limit = 3800
    for i in range(0, len(msg), limit):
        await u.message.reply_text(msg[i:i + limit])
...
async def cmd_restore(u: Update, c: ContextTypes.DEFAULT_TYPE, show_buttons=True):
    ...
    if show_buttons:
        ...
        success = storage.restore_from_leaderboard(target_uid, restore_scores=False, clear_scores=True)
        if success:
...
async def notify_new_top_speed(c: ContextTypes.DEFAULT_TYPE):
    message = "نزلت دزة جديدة اكتب أعلى الدزات عشان تشوفها"
    for user_id in storage.data.get("users", {}).keys():
        try:
            await c.bot.send_message(chat_id=int(user_id), text=message)
        except Exception as e:
            print(f"[TOP_SPEED_NOTIFY] Failed to send to user {user_id}: {e}")
...
async def cmd_stats(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    total_users = len(storage.data.get("users", {}))
    total_chats = len(storage.data.get("chats", {}))
    banned_count = len(storage.banned_data.get("banned", []))
...
async def cmd_banned_list(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    banned_ids = storage.banned_data.get("banned", [])
...
async def handle_msg(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    # معالجة حالات إدارة الصدارة (leaderboard_management_state) - التحقق أولاً قبل النص
    if uid in leaderboard_management_state:
        state = leaderboard_management_state[uid]
        stage = state.get("stage")
        
        # استقبال ملف الصدارة
        if u.message and u.message.document:
            file_id = u.message.document.file_id
            file_name = u.message.document.file_name or f"leaderboard_{stage}.json"
            
            if stage == "waiting_mobile":
                state["files"]["mobile_file_id"] = file_id
                state["files"]["mobile_name"] = file_name
                state["stage"] = "waiting_external"
                await u.message.reply_text("تم حفظ ملف صدارة الجوال\n\nالآن أرسل لي ملف صدارة الخارجي")
                return
            elif stage == "waiting_external":
                state["files"]["external_file_id"] = file_id
                state["files"]["external_name"] = file_name
                
                # تحميل الملفات وتطبيقها
                await apply_leaderboard_files(u, c, uid, state["files"])
                del leaderboard_management_state[uid]
                return
        
        # إذا كانت الرسالة نصية في انتظار ملف، أرسل رسالة تذكيرية
        if u.message and u.message.text:
            await u.message.reply_text(f"يرجى إرسال ملف JSON، لا رسالة نصية\n\nحالة البوت: (في انتظار {stage})")
        return

    # معالجة خطوات إضافة توب الدزات (قبل فحص النص)
    if uid in add_top_speed_state:
        state = add_top_speed_state[uid]
        step = state["step"]

        if step == "waiting_username":
            if not u.message.text:
                await u.message.reply_text("الرجاء إدخال يوزر صحيح يبدأ بـ @")
                return
            ...
            await u.message.reply_text("حلو طيب اعطيني دليله إو اي اثبات")
            return

        elif step == "waiting_video":
            state["message_id"] = u.message.message_id
            state["step"] = "waiting_speed"
            await u.message.reply_text("السرعة حقته كانت كم؟")
            return

        elif step == "waiting_speed":
            if not u.message.text:
                await u.message.reply_text("الرجاء إدخال رقم صحيح للسرعة")
                return
            text = u.message.text.strip()
            try:
                speed = float(text)
                ...
                return
            state["speed"] = speed
            state["step"] = "waiting_section"
            await u.message.reply_text("وش كان القسم")
            return

        elif step == "waiting_section":
            if not u.message.text:
                await u.message.reply_text("الرجاء إدخال اسم القسم")
                return
            text = u.message.text.strip()
            state["section"] = text
            storage.add_top_speed(state["username"], state["speed"], state["section"], state["message_id"])
            storage.save(force=True)
            await notify_new_top_speed(c)
            del add_top_speed_state[uid]
            await u.message.reply_text("خلاص تمام حطيته مع صدارة الدزات")
            return
    ...
    # حالة توقف المجموعة (المالك الأساسي فقط)
    if u.effective_chat.type in ['group', 'supergroup'] and storage.is_chat_paused(cid):
        if text == "شغل هنا" and storage.is_main_owner(uid):
            storage.set_chat_paused(cid, False)
            await u.message.reply_text("تم تشغيل البوت في هذه المجموعة")
            return
        else:
            return
    ...
    if text == "تحويل المقياس":
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("مقياس البوت", callback_data="scale_bot")],
            [InlineKeyboardButton("مقياس مونكي تايب", callback_data="scale_monkeytime")]
        ])
        current_scale = user_scale_mode.get(uid, "bot")
        current_label = "مقياس البوت" if current_scale == "bot" else "مقياس مونكي تايب"
        await u.message.reply_text(f"اختر مقياس السرعة (الحالي: {current_label}):", reply_markup=keyboard)
        return

    # معالجة أوامر تغيير نوع كرر
    if text in ["كرر حرفين", "كرر حرفين ", " كرر حرفين"]:
        user_repeat_type[uid] = "حرفين"
        await u.message.reply_text("تم الحين الكلمات صارت من حرفين")
        return
    
    if text in ["كرر ثلاث حروف", "كرر ثلاثه حروف", "كرر ثلاث حروف ", "كرر ثلاثه حروف "]:
        user_repeat_type[uid] = "ثلاث"
        await u.message.reply_text("تم الحين الكلمات صارت من ثلاث حروف")
        return
    
    if text in ["كرر اربع حروف", "كرر اربعه حروف", "كرر اربع حروف ", "كرر اربعه حروف "]:
        user_repeat_type[uid] = "اربع"
        await u.message.reply_text("تم الحين الكلمات صارت من اربع حروف")
        return
    
    if text in ["كرر خمس حروف", "كرر خمسه حروف", "كرر خمس حروف ", "كرر خمسه حروف "]:
        user_repeat_type[uid] = "خمس"
        await u.message.reply_text("تم الحين الكلمات صارت من خمس حروف")
        return
    
    if text in ["كرر مختلط", "كرر مختلط "]:
        user_repeat_type[uid] = "مختلط"
        await u.message.reply_text("تم الحين الكلمات صارت مختلطة من كل الأنواع")
        return
    
    if text in ["كرر قديم", "كرر قديم "]:
        if uid in user_repeat_type:
            del user_repeat_type[uid]
        await u.message.reply_text("تم رجعناه نفس قبل")
        return
    ...
    if text.startswith("ازل بدون حظر"):
        if not (storage.is_main_owner(uid) or storage.is_owner(uid)):
            await u.message.reply_text("هذا الأمر للملاك والمالك الأساسي فقط")
            return
        await cmd_remove_from_leaderboard_without_ban(u, c, text)
        return

    if text.startswith("حول ارقام") or text.startswith("حول أرقام"):
        if not (storage.is_main_owner(uid) or storage.is_owner(uid)):
            await u.message.reply_text("هذا الأمر للملاك والمالك الأساسي فقط")
            return
        await cmd_transfer_scores_to_external(u, c, text)
        return

    # معالجة أمر تحميل/تثبيت الصدارة (فقط للمالك الأساسي)
    if text == "تحميل صدارة":
        if not storage.is_main_owner(uid):
            await u.message.reply_text("هذا الأمر فقط للمالك الأساسي")
            return
        await cmd_leaderboard_management(u, c)
        return

    # معالجة أمر إزالة مستخدم من الصدارة (فقط للمالك الأساسي)
    if text.startswith("ازل"):
        if not (storage.is_main_owner(uid) or storage.is_owner(uid)):
            await u.message.reply_text("هذا الأمر للملاك والمالك الأساسي فقط")
            return
        await cmd_remove_from_leaderboard(u, c, text)
        return

    # معالجة أمر إضافة رقم يدوي للصدارة (فقط للمالك الأساسي)
    if text.startswith("اضف ") and " لصدارة " in text and " قسم " in text and " بسرعة " in text:
        if not storage.is_main_owner(uid):
            await u.message.reply_text("هذا الأمر فقط للمالك الأساسي")
            return
        await cmd_add_to_leaderboard(u, c, text)
        return
    ...
    if text.startswith("ارجع"):
        ...
        if not (storage.is_main_owner(uid) or storage.is_owner(uid)):
            await u.message.reply_text("هذا الأمر فقط للملاك والمالك الأساسي")
            return
        await cmd_restore(u, c, show_buttons=False)
        return

    # معالجة أمر إضافة إلى توب الدزات (للمالك الأساسي فقط)
    if text == "اضافه إلى توب الدزات":
        if not storage.is_main_owner(uid):
            await u.message.reply_text("هذا الأمر فقط للمالك الأساسي")
            return
        add_top_speed_state[uid] = {"step": "waiting_username"}
        await u.message.reply_text("اعطيني يوزر المستخدم")
        return

    # معالجة خطوات إضافة توب الدزات
    # معالجة أمر عرض أعلى الدزات
    if text in ["أعلى الدزات", "توب الدزات", "اعلى الدزات", "توب الدزات"]:
        top_speeds = storage.get_top_speeds()
        if not top_speeds:
            await u.message.reply_text("لا توجد دزات مسجلة حالياً")
            return

        keyboard = []
        for i, speed_data in enumerate(top_speeds):
            username = speed_data["username"]
            speed = speed_data["speed"]
            section = speed_data["section"]
            button_text = f"{username} , {speed:.2f} WPM , {section}"
            keyboard.append([InlineKeyboardButton(button_text, callback_data=f"top_speed_{i}")])

        reply_markup = InlineKeyboardMarkup(keyboard)
        await u.message.reply_text("أعلى الدزات:", reply_markup=reply_markup)
        return
    ...
    # نزل مالك [أيدي أو يوزر]
    if text.startswith("نزل مالك ") and storage.is_main_owner(uid):
        target_input = text.replace("نزل مالك ", "").strip().lstrip('@')
        if not target_input:
            await u.message.reply_text("الرجاء أدخل الأيدي أو اليوزر: `نزل مالك 123456789`", parse_mode="Markdown")
            return
        ...
        try:
            ...
            if target_uid is None:
                await u.message.reply_text("لم أجد هذا اليوزر في قاعدة البيانات")
                return

        if not storage.is_owner(target_uid):
            await u.message.reply_text(f"الأيدي `{target_uid}` ليس مالك")
            return

        storage.remove_owner(target_uid)
        await u.message.reply_text(f"تم تنزيل الأيدي `{target_uid}` من رتبة مالك")
    ...
    if text.startswith("تنزيل مالك ") and storage.is_main_owner(uid):
        ...
        storage.remove_owner(target_uid)
        await u.message.reply_text(f"تم تنزيل الأيدي `{target_uid}` من رتبة مالك", parse_mode="Markdown")
    ...
    if storage.get_broadcast_mode(uid):
        ...
        # إرسال للمستخدمين
        for user_id in list(storage.data["users"].keys()):
            try:
        ...
        # تنظيف المستخدمين الذين حظروا البوت أو حذفوا حساباتهم
        if invalid_user_ids:
            for bad_uid in invalid_user_ids:
                if bad_uid in storage.data.get("users", {}):
                    storage.data["users"].pop(bad_uid, None)
                    removed_users += 1
            if removed_users > 0:
                storage.mark_dirty()
                storage.save(force=True)
        ...
        if removed_users > 0:
            msg += f"\nتمت إزالة {removed_users} مستخدم غير متاح من قائمة الإذاعة"
            ...
            for section, session in auto_sessions.items():
                ...
                if matched:
                    ...
                    if should_save_speed:
                        ...
                        # استثناء: في وضع مونكي تايب لا تحفظ الصدارة
                        leaderboard_sections = ["كرر", "ويكي", "شك", "جب", "جش", "جمم"]
                        if section_type in leaderboard_sections and accuracy == 100.0:
                            ...
                            if section_type == "جب" and user_jab_type.get(uid) == "حرفين":
                                ...
                                is_flagged = storage.update_score(uid, section_type, wpm, device_type, accuracy)
                                # إذا تم فلترة النقطة بسبب سرعة عالي على جوال، أرسل رسالة للمالك للموافقة
                                if is_flagged:
                                    # حفظ معلومات النقطة قيد المراجعة
                                    score_key = f"{uid}_{section_type}_{device_type}"
                                    pending_high_speed_scores[score_key] = {
                                        "uid": uid,
                                        "typ": section_type,
                                        "wpm": wpm,
                                    ...
                                    # إرسال الرسالة للمالك مع الأزرار
                                    user_display = f"@{u.effective_user.username}" if u.effective_user.username else u.effective_user.first_name
                                    msg_to_owner = f"خالد فيه واحد معه اشتباه غش ذا هو {user_display} (ID: {uid})\n\nالقسم: {section_type}\nالسرعة: {wpm:.1f} WPM\nالجهاز: {device_type}"
                                    
                                    keyboard = InlineKeyboardMarkup([
                                        [
                                            InlineKeyboardButton("رفض الرقم", callback_data=f"score_reject_{score_key}"),
                                            InlineKeyboardButton("سماح للرقم", callback_data=f"score_approve_{score_key}")
                                        ]
                                    ])
                                    
                                    try:
                                        await c.bot.send_message(chat_id=OWNER_ID, text=msg_to_owner, reply_markup=keyboard)
                                    except Exception as e:
                                        print(f"Error sending score approval request to owner: {e}")
                                    
                                    # إرسال رسالة للمستخدم بأن الرقم صار معلقاً
                                    await u.message.reply_text(
                                        f"سرعتك: {wpm:.1f}\n"
                                        "انت رقمك فوق 190 وانت تصنيفك في توب الجوال رقمك للحين معلق ارسل تصويرك ل @iv511vi عشان يقبله"
                                    )
                    ...
                    if should_save_speed:
                        # حساب وضع العرض (bot / monkeytime)
                        is_monkeytime = user_scale_mode.get(uid) == "monkeytime"
                        _elapsed_safe = max(elapsed, 0.01)
                        display_wpm = (word_count / _elapsed_safe) * 55 if is_monkeytime else wpm
                        if is_monkeytime and user_accuracy_on_local:
                            _penalty_ref = convert_repeat_pattern_to_words(orig) if ('(' in orig and ')' in orig) else orig
                            _wc = count_wrong_chars(_penalty_ref, text)
                            if _wc > 0:
                                display_wpm = max(0.0, display_wpm - _wc * random.uniform(2.5, 4.0))
                        reply_txt = get_speed_message(username, display_wpm, elapsed, neutral=is_monkeytime)
                        if accuracy is not None and user_accuracy_on_local:  # استخدام المتغير المحفوظ
                            reply_txt += f"\nالدقة : {accuracy}%"
                        if is_monkeytime and user_accuracy_on_local and accuracy != 100.0:
                            raw_wpm = (count_words_for_wpm(text) / _elapsed_safe) * 55
                            reply_txt += f"\nالسرعة الخام: {raw_wpm:.2f}"
    ...
    # أمر إيقاف البوت في هذه القروب (المالك الأساسي فقط)
    if text == "وقف هنا":
        if not storage.is_main_owner(uid):
            await u.message.reply_text("هذا الأمر للمالك الأساسي فقط")
            return
        if u.effective_chat.type not in ['group', 'supergroup']:
            await u.message.reply_text("هذا الأمر يعمل في المجموعات فقط")
            return

        storage.set_chat_paused(cid, True)
        await u.message.reply_text("تم إيقاف البوت في هذه المجموعة. اكتب 'شغل هنا' لتشغيله مرة أخرى")
        return

    # أمر تشغيل البوت في هذه القروب (المالك الأساسي فقط)
    if text == "شغل هنا":
        if not storage.is_main_owner(uid):
            await u.message.reply_text("هذا الأمر للمالك الأساسي فقط")
            return
    ...
    # طرد الكل من المجموعة الحالية (للمالك الأساسي فقط)
    if text == "طرد الكل":
        if not storage.is_main_owner(uid):
            await u.message.reply_text("هذا الأمر للمالك الأساسي فقط")
            return
        ...
        if not await bot_can_kick_members(c.bot, cid):
            await u.message.reply_text("البوت ما عنده صلاحية طرد في هذه المجموعة")
            return
        ...
        for uid_str in storage.data.get("users", {}).keys():
            ...
            ok, _ = await kick_user_from_chat(c.bot, cid, target_uid)
            if ok:
                kicked += 1
            else:
                skipped += 1

        await u.message.reply_text(f"تم تنفيذ طرد الكل\n\nتم طرد: {kicked}\nتخطي: {skipped}")
        return

    # طرد كل من مجموعة محددة بالأيدي (لا يشترط وجود المنفذ داخل المجموعة)
    if text.startswith("طرد كل اللي في"):
        if not (storage.is_main_owner(uid) or storage.is_owner(uid) or storage.is_admin(uid)):
            await u.message.reply_text("هذا الأمر للمشرفين فقط")
            return

        m = re.match(r"^طرد كل اللي في\s+(-?\d+)\s*$", text)
        if not m:
            await u.message.reply_text("الاستخدام: طرد كل اللي في [أيدي المجموعة]")
            return

        target_chat_id = int(m.group(1))
        if not await bot_can_kick_members(c.bot, target_chat_id):
            await u.message.reply_text("البوت ما عنده صلاحية طرد في هذه المجموعة")
            return
        ...
        for uid_str in storage.data.get("users", {}).keys():
            ...
            ok, _ = await kick_user_from_chat(c.bot, target_chat_id, target_uid)
            if ok:
                kicked += 1
            else:
                skipped += 1

        await u.message.reply_text(f"تم تنفيذ الطرد في المجموعة {target_chat_id}\n\nتم طرد: {kicked}\nتخطي: {skipped}")
        return

    # طرد مستخدم محدد من مجموعة محددة
    if text.startswith("طرد ") and " من مجموعة " in text:
        if not (storage.is_main_owner(uid) or storage.is_owner(uid) or storage.is_admin(uid)):
            await u.message.reply_text("هذا الأمر للمشرفين فقط")
            return

        m = re.match(r"^طرد\s+(.+?)\s+من مجموعة\s+(-?\d+)\s*$", text)
        if not m:
            await u.message.reply_text("الاستخدام: طرد [@يوزر أو أيدي] من مجموعة [أيدي المجموعة]")
            return

        target_raw = m.group(1).strip()
        target_chat_id = int(m.group(2))

        target_uid, _, display_name = resolve_target_user(target_raw)
        if target_uid is None and target_raw.startswith("@"):
            try:
                user_chat = await c.bot.get_chat(target_raw)
                target_uid = user_chat.id
                display_name = target_raw
            except Exception:
                pass

        if target_uid is None:
            await u.message.reply_text("لم أستطع تحديد المستخدم المطلوب")
            return

        if storage.is_main_owner(target_uid):
            await u.message.reply_text("لا يمكن طرد المالك الأساسي")
            return
        ...
        ok, reason = await kick_user_from_chat(c.bot, target_chat_id, target_uid)
        if ok:
            await u.message.reply_text(f"تم طرد {display_name} من المجموعة {target_chat_id}")
        else:
            if reason == "protected":
                await u.message.reply_text("ما أقدر أطرد هذا المستخدم لأنه أدمن/منشئ")
            elif reason == "absent":
                await u.message.reply_text("المستخدم غير موجود حالياً في المجموعة")
            else:
                await u.message.reply_text("فشل الطرد، تأكد من صلاحيات البوت وصحة البيانات")
        return
    ...
    if text == "روابط القروبات":
        ...
        chat_links = []
        for chat_id_str, chat_info in storage.data.get("chats", {}).items():
            try:
                chat_obj = await c.bot.get_chat(int(chat_id_str))
                invite_link = getattr(chat_obj, "invite_link", None)
                if not invite_link:
                    if chat_id_str.startswith("-100"):
                        invite_link = f"https://t.me/c/{chat_id_str[4:]}"
                    else:
                        invite_link = "غير متوفر"

                title = chat_info.get("title", "غير معروف")
                chat_links.append(f"{title} ({chat_id_str})\n{invite_link}")
            except Exception:
                title = chat_info.get("title", "غير معروف")
                chat_links.append(f"{title} ({chat_id_str})\nرابط غير متوفر")

        if not chat_links:
            await u.message.reply_text("لا توجد مجموعات مسجلة حالياً")
            return

        # تقسيم الناتج إذا كان طويلًا
        chunk = "\n\n".join(chat_links)
        if len(chunk) > 4000:
            parts = [chunk[i:i+4000] for i in range(0, len(chunk), 4000)]
            for part in parts:
                await u.message.reply_text(part)
        else:
            await u.message.reply_text(chunk)

        storage.log_cmd("روابط القروبات")
        return

    # عرض أيديات الإشراف
    if text in ["أيدي الإشراف", "ايدي الاشراف", "ايدي الإشراف", "ايدي مشرفين", "أيدي مشرفين", "ايدي الاشراف", "أيدياشراف"]:
        if not (storage.is_main_owner(uid) or storage.is_owner(uid) or storage.is_admin(uid)):
            await u.message.reply_text("هذا الأمر للمشرفين فقط")
            return
        ...
        if owners:
            msg += "الملاك:\n"
            for owner_id in owners:
                owner_data = storage.data.get("users", {}).get(str(owner_id), {})
                username = owner_data.get("username")
                first_name = owner_data.get("first_name") or owner_data.get("name")
                if username:
                    msg += f"• {owner_id} | @{username}\n"
                elif first_name:
                    msg += f"• {owner_id} | {first_name}\n"
                else:
                    msg += f"• {owner_id}\n"
            msg += "\n"
        ...
        if admins:
            msg += "المشرفون:\n"
            for admin_id in admins:
                admin_data = storage.data.get("users", {}).get(str(admin_id), {})
                username = admin_data.get("username")
                first_name = admin_data.get("first_name") or admin_data.get("name")
                if username:
                    msg += f"• {admin_id} | @{username}\n"
                elif first_name:
                    msg += f"• {admin_id} | {first_name}\n"
                else:
                    msg += f"• {admin_id}\n"
    ...
    if text == "المزالين":
        ...
        removed_list = storage.leaderboard_data.get("removed_from_leaderboard", {})
    ...
    if text.startswith("حذف ") and ("من صدارة" in text) and ("قسم" in text):
        ...
        try:
            import re
            pattern = r"حذف\s+(@?\S+)\s+من\s+صدارة\s+(?:ال)?(جوال|خارجي)\s+قسم\s+(\S+)"
            ...
            device_type = "جوال" if device_type_text == "جوال" else "خارجي"
    ...
    if text.startswith("ارجع "):
        ...
        try:
            ...
            if storage.restore_from_leaderboard(target_uid, restore_scores=False, clear_scores=True):
                display_name = f"@{target_username}" if target_username else target_name
    ...
    if text == "غير الرق":
        async with chat_locks[cid]:
            ...
            keyboard = [
                ...
                [InlineKeyboardButton("رق سهل جدا (0-10) اكتبها لفظاً", callback_data="رق_سهل_جدا")]
            ]
            reply_markup = InlineKeyboardMarkup(keyboard)
            await u.message.reply_text("إذا تبي تغير نهج الأرقام اختر:\n\nلفظ: كلمات عربية جاهزة (واحد اثنين ثلاثه...)\nصعبة: أرقام من 0-1000 تكتبها لفظاً\nسهلة: أرقام من 0-100 تكتبها لفظاً\nسهل جدا: أرقام من 0-10 تكتبها لفظاً", reply_markup=reply_markup)
        return
    ...
    if text.startswith("رق "):
        async with chat_locks[cid]:
            ...
            if num >= 50:
                await u.message.reply_text("تحت الخمسين يبو هد")
                return

            if not (1 <= num < 50):
                await u.message.reply_text("الرجاء إدخال رقم بين 1 و 49")
                return
            ...
            if saved_type == "لفظ":
                ...
                await u.message.reply_text(f"اكتب الأرقام لفظاً:\n\n{numbers_str}")
            elif saved_type == "سهل جدا":
                numbers, numbers_str = generate_very_easy_numbers_sentence(num)
                storage.del_session(cid, "رق_سهل_جدا")
                storage.save_session(uid, cid, "رق_سهل_جدا", numbers_str, time.time(), sent=True)
                await u.message.reply_text(f"اكتب الأرقام لفظاً:\n\n{numbers_str}")
            else:
                numbers, numbers_str = generate_hard_numbers_sentence(num)
                storage.del_session(cid, "رق_صعبة")
    ...
    if command == "رق" or text == "رق":
        async with chat_locks[cid]:
            ...
            if saved_type == "لفظ":
                ...
                count = pref_count or random.randint(5, 10)
                numbers, numbers_str = generate_very_easy_numbers_sentence(count)
                storage.del_session(cid, "رق_سهل_جدا")
                storage.save_session(uid, cid, "رق_سهل_جدا", numbers_str, time.time(), sent=True)
                await u.message.reply_text(f"اكتب الأرقام لفظاً:\n\n{numbers_str}")
            else:
        ...
        try:
            ...
            if typ in ["جمم", "ويكي", "صج", "شك", "جش", "قص", "نص", "جب", "حر"]:
                ...
                if match_numbers(orig, text):
                    ...
                    matched = False
            elif typ in ["رق_صعبة", "رق_سهلة", "رق_سهل_جدا"]:
                if match_numbers(orig, text):
                    matched = True
                    accuracy = 100.0  # إذا تطابقت الأرقام تماماً، الدقة 100%
                else:
    ...
    if best_match:
            ...
            if game and text != "جاهز" and text != "تنسحب":
                ...
                if game_id not in pending_match_answers:
                        ...
                        if other_time < current_time:
                            ...
                            if should_save_other:
                                ...
                                if chosen_section in leaderboard_sections and accuracy == 100.0:
                                    is_flagged = storage.update_score(other_uid, chosen_section, other_wpm, other_device_type, accuracy)
                                    if is_flagged:
                                        # حفظ النقطة قيد المراجعة وإرسال للمالك
                                        score_key = f"{other_uid}_{chosen_section}_{other_device_type}"
                                        other_player = storage.data.get("users", {}).get(str(other_uid), {})
                                        other_username = other_player.get("username") or other_player.get("first_name", "مستخدم")
                                        ...
                                        user_display = f"@{other_username}" if other_player.get("username") else other_username
                                        msg_to_owner = f"خالد فيه واحد معه اشتباه غش ذا هو {user_display} (ID: {other_uid})\n القسم: {chosen_section}\nالسرعة: {other_wpm:.1f} WPM\nالجهاز: {other_device_type}"
                                        ...
                                        # تنبيه المستخدم نفسه أن رقمه صار معلق
                                        try:
                                            await context.bot.send_message(
                                                chat_id=other_uid,
                                                text=(
                                                    f"سرعتك: {other_wpm:.1f}\n"
                                                    "انت رقمك فوق 190 وانت تصنيفك في توب الجوال رقمك للحين معلق ارسل تصويرك ل @iv511vi عشان يقبله"
                                                )
                                            )
                                        except:
                    ...
                    if should_save_uid:
                        ...
                        if chosen_section in leaderboard_sections and accuracy == 100.0:
                            ...
                            if chosen_section == "جب" and user_jab_type.get(uid) == "حرفين":
                                ...
                                is_flagged = storage.update_score(uid, chosen_section, wpm, device_type, accuracy)
                                if is_flagged:
                                    # حفظ النقطة قيد المراجعة وإرسال للمالك
                                    score_key = f"{uid}_{chosen_section}_{device_type}"
                                    player = storage.data.get("users", {}).get(str(uid), {})
                                    player_username = player.get("username") or player.get("first_name", "مستخدم")
                                    ...
                                    user_display = f"@{player_username}" if player.get("username") else player_username
                                    msg_to_owner = f"خالد فيه واحد معه اشتباه غش ذا هو {user_display} (ID: {uid})\n القسم: {chosen_section}\nالسرعة: {wpm:.1f} WPM\nالجهاز: {device_type}"
                                    ...
                                    # تنبيه صاحب الرقم أن نتيجته معلقة
                                    await u.message.reply_text(
                                        f"سرعتك: {wpm:.1f}\n"
                                        "انت رقمك فوق 190 وانت تصنيفك في توب الجوال رقمك للحين معلق ارسل تصويرك ل @iv511vi عشان يقبله"
                                    )
                    ...
                    if not is_loser:
                        ...
                        username = u.effective_user.username or u.effective_user.first_name or "مستخدم"
                        _mt_match = user_scale_mode.get(uid) == "monkeytime"
                        _disp_wpm = (word_count / elapsed) * 55 if _mt_match else wpm
                        if _mt_match and is_accuracy_enabled_for(uid):
                            _penalty_ref = convert_repeat_pattern_to_words(orig) if ('(' in orig and ')' in orig) else orig
                            _wc = count_wrong_chars(_penalty_ref, text)
                            if _wc > 0:
                                _disp_wpm = max(0.0, _disp_wpm - _wc * random.uniform(2.5, 4.0))
                        speed_msg = get_speed_message(username, _disp_wpm, elapsed, neutral=_mt_match)
                        if accuracy is not None and is_accuracy_enabled_for(uid):
                               speed_msg += f"\nالدقة : {accuracy}%"
                        if _mt_match and is_accuracy_enabled_for(uid) and accuracy != 100.0:
                            speed_msg += f"\nالسرعة الخام: {(count_words_for_wpm(text) / elapsed) * 55:.2f}"
        ...
        if is_fully_accepted:
            ...
            if score_typ in leaderboard_sections:
                ...
                if score_typ == "جب" and user_jab_type.get(uid) == "حرفين":
                    ...
                    is_flagged = storage.update_score(uid, score_typ, wpm, device_type, accuracy)
                    if is_flagged:
                        # حفظ النقطة قيد المراجعة وإرسال للمالك
                        score_key = f"{uid}_{score_typ}_{device_type}"
                        user_info = storage.data.get("users", {}).get(str(uid), {})
                        client_username = user_info.get("username") or user_info.get("first_name", "مستخدم")
                        ...
                        user_display = f"@{client_username}" if user_info.get("username") else client_username
                        msg_to_owner = f"خالد فيه واحد معه اشتباه غش ذا هو {user_display} (ID: {uid})\n القسم: {score_typ}\nالسرعة: {wpm:.1f} WPM\nالجهاز: {device_type}"
        ...
        # حساب وضع العرض (bot / monkeytime) - مرة واحدة لكل المواضع
        _is_mt = user_scale_mode.get(uid) == "monkeytime"
        display_wpm = (word_count / elapsed) * 55 if _is_mt else wpm
        _raw_wpm_addon = ""
        if _is_mt and is_accuracy_enabled_for(uid):
            if accuracy != 100.0:
                _raw_wpm_addon = f"\nالسرعة الخام: {(count_words_for_wpm(text) / elapsed) * 55:.2f}"
            _penalty_ref = convert_repeat_pattern_to_words(orig) if ('(' in orig and ')' in orig) else orig
            _wc = count_wrong_chars(_penalty_ref, text)
        ...
        if round_data:
            ...
            if storage.is_round_finished(cid):
                ...
                reply = get_speed_message(mention, display_wpm, elapsed, neutral=_is_mt)
                if accuracy is not None and user_accuracy_enabled:
                    reply += f"\nالدقة : {accuracy}%"
                reply += _raw_wpm_addon
...
async def display_leaderboard(query, device_type):
    ...
    is_mt = user_scale_mode.get(uid) == "monkeytime"
    scale_label = "مونكي تايب" if is_mt else "البوت"
    types = ["كرر", "ويكي", "شك", "جب", "جش", "حر"]
    sections = []
    ...
    for typ in types:
        ...
        if lb:
            ...
            for i, (uid_str, username, first_name, wpm) in enumerate(lb[:5], 1):
                display = f"@{username}" if username else first_name
                if is_mt:
                    # كرر يحتاج تحويل ألطف في العرض حتى لا تنخفض الأرقام بشكل مبالغ.
                    display_wpm = (wpm * 55 / 60) if typ == "كرر" else max(0.0, (wpm - 20) / 60 * 55)
                else:
                    display_wpm = wpm
                s += f"{i}. {display}: {display_wpm:.2f} WPM\n"
            sections.append(s)

    if sections:
        scale_note = "\n<i>(مونكي تايب)</i>" if is_mt else "\n<i>(مقياس البوت)</i>"
        msg = f"<b>صدارة {device_type}</b>{scale_note}\n\n" + "\n".join(sections)
...
async def handle_callback(u: Update, c: ContextTypes.DEFAULT_TYPE):
    ...
    # معالجة الموافقة على النقاط العالية - فقط المالك
    if query.data.startswith("score_approve_") or query.data.startswith("score_reject_"):
        # تحقق أن المستخدم هو المالك الأساسي
        if not storage.is_main_owner(uid):
            await query.answer("هذا الأمر فقط للمالك الأساسي", show_alert=True)
            return
        
        await query.answer()
        
        # استخراج المفتاح والإجراء
        if query.data.startswith("score_approve_"):
            score_key = query.data[len("score_approve_"):]
            is_approve = True
        else:
            score_key = query.data[len("score_reject_"):]
            is_approve = False
        
        # الحصول على معلومات النقطة
        score_info = pending_high_speed_scores.get(score_key)
        if not score_info:
            await query.edit_message_text("معلومات النقطة غير موجودة")
            return
        
        user_id = score_info["uid"]
        section = score_info["typ"]
        wpm = score_info["wpm"]
        device_type = score_info["device_type"]
        username = score_info["username"]
        
        if is_approve:
            # اعتماد الرقم: حفظه في الصدارة ثم إزالة التعليق
            if "flagged_high_speed_mobile" not in storage.data:
                storage.data["flagged_high_speed_mobile"] = {}
            
            score_key_in_data = f"{user_id}_{section}_{device_type}"
            if "scores" not in storage.leaderboard_data:
                storage.leaderboard_data["scores"] = {}

            old_score = storage.leaderboard_data["scores"].get(score_key_in_data, 0)
            storage.leaderboard_data["scores"][score_key_in_data] = max(old_score, wpm)
            storage.data["flagged_high_speed_mobile"].pop(score_key_in_data, None)
            storage.save_leaderboard()
            storage.mark_dirty()
            storage.save(force=True)
            
            await query.edit_message_text(f"تمت الموافقة على رقم {username}\nالرقم الآن يحسب في الصدارة")
        else:
            # الرفض - حذف الرقم المعلق نهائياً من الصدارة
            score_key_in_data = f"{user_id}_{section}_{device_type}"
            if "scores" in storage.leaderboard_data:
                storage.leaderboard_data["scores"].pop(score_key_in_data, None)
            if "flagged_high_speed_mobile" in storage.data:
                storage.data["flagged_high_speed_mobile"].pop(score_key_in_data, None)
            storage.save_leaderboard()
            storage.mark_dirty()
            storage.save(force=True)
            await query.edit_message_text(f"تم رفض رقم {username}\nالرقم لن يظهر في الصدارة")
        
        # حذف من قائمة الانتظار
        pending_high_speed_scores.pop(score_key, None)
        return

    # معالجة أزرار إدارة الصدارة (تحميل/تثبيت)
    if query.data == "leaderboard_download":
        # إرسال ملفات الصدارة الحالية
        if not storage.is_main_owner(uid):
            await query.answer("فقط المالك الأساسي يستطيع تحميل الصدارة", show_alert=True)
            return
        
        await query.answer()
        await query.edit_message_text("⏳ جاري تحضير الملفات...")
        ...
        try:
            ...
            for key, score in storage.leaderboard_data.get("scores", {}).items():
                if "_جوال" in key:
                    # إزالة "_جوال" من المفتاح
                    clean_key = key.replace("_جوال", "")
                    mobile_scores[clean_key] = score
                elif "_خارجي" in key:
                    # إزالة "_خارجي" من المفتاح
                    clean_key = key.replace("_خارجي", "")
                    external_scores[clean_key] = score
            
            # حفظ الملفات مؤقتاً
            mobile_file = "leaderboard_mobile.json"
            external_file = "leaderboard_external.json"
            
            with open(mobile_file, 'w', encoding='utf-8') as f:
                json.dump(mobile_scores, f, ensure_ascii=False, indent=2)
            
            with open(external_file, 'w', encoding='utf-8') as f:
                json.dump(external_scores, f, ensure_ascii=False, indent=2)
            
            # إرسال الملفات
            with open(mobile_file, 'rb') as f:
                await query.message.reply_document(f, caption="📱 صدارة الجوال الحالية")
            
            with open(external_file, 'rb') as f:
                await query.message.reply_document(f, caption="🖥️ صدارة الخارجي الحالية")
            
            await query.message.reply_text("تم تحميل ملفات الصدارة بنجاح")
            
        except Exception as e:
            await query.message.reply_text(f"حدث خطأ: {str(e)}")
            print(f"[LEADERBOARD] Error downloading files: {e}")
        
        return
    
    if query.data == "leaderboard_upload":
        # طلب تثبيت ملفات صدارة جديدة
        if not storage.is_main_owner(uid):
            await query.answer("فقط المالك الأساسي يستطيع حفظ الصدارة", show_alert=True)
            return
        
        await query.answer()
        
        # تعيين الحالة لاستقبال الملفات
        leaderboard_management_state[uid] = {
            "stage": "waiting_mobile",
            "files": {}
        }
        
        await query.edit_message_text("حلو ارسل لي الملف حق صدارة الجوال")
        return
    ...
    # معالجة أزرار أعلى الدزات
    if query.data.startswith("top_speed_"):
        index = int(query.data.split("_")[-1])
        top_speeds = storage.get_top_speeds()
        if index < len(top_speeds):
            speed_data = top_speeds[index]
            username = speed_data["username"]
            speed = speed_data["speed"]
            section = speed_data["section"]
            message_id = speed_data.get("message_id")
            if message_id and OWNER_ID:
                try:
                    await c.bot.forward_message(chat_id=query.from_user.id, from_chat_id=OWNER_ID, message_id=message_id)
                except Exception as e:
                    await query.message.reply_text(f"حدث خطأ في إرسال الدليل: {e}")
                    return
            await asyncio.sleep(1)
            await query.message.reply_text(f"ذي دزة {username} ما شاء الله اي دزة عندك تواصل مع @iv511vi")
        return

    # معالجة أزرار تحويل المقياس
    if query.data == "scale_bot":
        await query.answer()
        user_scale_mode[uid] = "bot"
        await query.edit_message_text("تم تغيير المقياس إلى مقياس البوت\n\nتنبيه: الصدارة والسرعات المعروضة لك الآن ستكون بمقياس البوت")
        return

    if query.data == "scale_monkeytime":
        await query.answer()
        user_scale_mode[uid] = "monkeytime"
        await query.edit_message_text("تم تغيير المقياس إلى مونكي تايب\n\nتنبيه: الصدارة والسرعات المعروضة لك الآن ستكون بمقياس مونكي تايب")
        return
    ...
    if query.data == "رق_سهل_جدا":
        storage.save_number_type(uid, "سهل جدا")
        numbers, numbers_str = generate_very_easy_numbers_sentence()
        storage.del_session(cid, "رق_سهل_جدا")
        storage.save_session(uid, cid, "رق_سهل_جدا", numbers_str, time.time(), sent=True)
        await query.edit_message_text(f"اكتب الأرقام لفظاً:\n\n{numbers_str}")
        return
...
async def cmd_leaderboard_management(u: Update, c: ContextTypes.DEFAULT_TYPE):
    """أمر تحميل/تثبيت ملفات الصدارة - يعرض أزرار الاختيار"""
    if not u.effective_user or not u.message:
        return
    ...
    keyboard = [
        [
            InlineKeyboardButton("📥 تحميل الصدارة", callback_data="leaderboard_download"),
            InlineKeyboardButton("☁️ حفظ الصدارة", callback_data="leaderboard_upload")
        ]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    
    await u.message.reply_text(
        "أرحب تبي تحفظ الصدارة ولا تبي تحمل صدارة؟",
        reply_markup=reply_markup
    )

async def cmd_remove_from_leaderboard(u: Update, c: ContextTypes.DEFAULT_TYPE, text: str):
    """أمر إزالة مستخدم من الصدارة (منع ظهوره)"""
    if not u.effective_user or not u.message:
        return
    
    uid = u.effective_user.id
    
    # استخراج المستخدم من الأمر: ازل 123 أو ازل @username
    parts = text.split(maxsplit=1)
    if len(parts) < 2:
        await u.message.reply_text("استخدم: ازل (رقم أو @يوزر)")
        return
    
    target = parts[1].strip()
    target_uid, _, target_name = resolve_target_user(target)
    
    if not target_uid:
        await u.message.reply_text(f"لم أجد المستخدم: {target}")
        return

    removed_types = storage.remove_from_leaderboard(target_uid)
    if removed_types:
        await u.message.reply_text(f"تم إزالة {target_name} من الصدارة\n\nسيبقى ممنوع من الظهور حتى تستخدم 'رجع {target_uid}' أو 'ارجع {target_uid}'")
    else:
        await u.message.reply_text("المستخدم غير موجود في الصدارة")
    print(f"[LEADERBOARD] Admin {uid} removed {target_uid} from leaderboard")

async def cmd_remove_from_leaderboard_without_ban(u: Update, c: ContextTypes.DEFAULT_TYPE, text: str):
    """حذف أرقام المستخدم الحالية من الصدارة بدون منعه من الظهور لاحقاً."""
    if not u.effective_user or not u.message:
        return

    parts = text.split(maxsplit=3)
    if len(parts) < 4:
        await u.message.reply_text("استخدم: ازل بدون حظر (الأيدي أو @اليوزر)")
        return

    target = parts[3].strip()
    target_uid, _, display_name = resolve_target_user(target)
    if not target_uid:
        await u.message.reply_text(f"لم أجد المستخدم: {target}")
        return

    was_removed = storage.is_removed_from_leaderboard(target_uid)
    removed_types = storage.clear_leaderboard_scores(target_uid, keep_backup=False, clear_removed_flag=True)

    if removed_types:
        msg = f"تم حذف أرقام {display_name} الحالية من الصدارة\n\n"
        msg += "أي رقم جديد يحققه سيُحسب مباشرة في الصدارة"
        await u.message.reply_text(msg)
        return

    if was_removed:
        await u.message.reply_text(f"تم فك منع {display_name} من الصدارة، لكن لا توجد له أرقام حالية للحذف")
        return

    await u.message.reply_text("المستخدم لا يملك أرقاماً حالية في الصدارة")

async def cmd_transfer_scores_to_external(u: Update, c: ContextTypes.DEFAULT_TYPE, text: str):
    """تحويل أرقام المستخدم إلى صدارة الخارجي."""
    if not u.effective_user or not u.message:
        return

    match = re.match(r"^حول\s+ارقام\s+(.+?)\s+لخارجي\s*$", text)
    if not match:
        match = re.match(r"^حول\s+أرقام\s+(.+?)\s+لخارجي\s*$", text)

    if not match:
        await u.message.reply_text("استخدم: حول ارقام (الأيدي أو @اليوزر) لخارجي")
        return
    ...
    moved_sections = storage.transfer_leaderboard_scores(target_uid, "خارجي")
    if moved_sections:
        msg = f"تم تحويل أرقام {display_name} إلى صدارة الخارجي\n\n"
        msg += f"الأقسام المنقولة: {', '.join(sorted(moved_sections))}"
        await u.message.reply_text(msg)
    else:
        storage.save_device_type(target_uid, "خارجي")
        storage.save(force=True)
        await u.message.reply_text(f"تم تحويل تصنيف {display_name} إلى خارجي، لكن لم أجد أرقاماً من الجوال لنقلها")

async def cmd_add_to_leaderboard(u: Update, c: ContextTypes.DEFAULT_TYPE, text: str):
    """إضافة رقم يدوي للصدارة: اضف (أيدي/يوزر) لصدارة (جوال/خارجي) قسم (القسم) بسرعة (الرقم)"""
    if not u.effective_user or not u.message:
        return

    pattern = r"^اضف\s+(.+?)\s+لصدارة\s+(جوال|خارجي)\s+قسم\s+(.+?)\s+بسرعة\s+([0-9]+(?:\.[0-9]+)?)\s*$"
    m = re.match(pattern, text)
    if not m:
        await u.message.reply_text("الاستخدام الصحيح:\nاضف (أيدي أو @يوزر) لصدارة (جوال أو خارجي) قسم (اسم القسم) بسرعة (رقم)")
        return

    target_raw = m.group(1).strip()
    device_type = m.group(2).strip()
    section = m.group(3).strip()
    speed = float(m.group(4))

    allowed_sections = ["كرر", "ويكي", "شك", "جب", "جش", "جمم", "حر"]
    if section not in allowed_sections:
        await u.message.reply_text(f"القسم غير مدعوم. الأقسام المتاحة: {', '.join(allowed_sections)}")
        return
    ...
    if target_raw.isdigit():
        ...
        target_first_name = user_data.get("first_name") or user_data.get("name")
        if not target_username and not target_first_name:
            try:
                chat = await c.bot.get_chat(target_uid)
                target_username = getattr(chat, "username", None)
                target_first_name = getattr(chat, "first_name", None) or getattr(chat, "title", None)
            except:
                pass
    else:
        username = target_raw.lstrip("@").strip()
        target_uid = get_user_id_by_username(username)
        if target_uid:
            ...
            try:
                chat = await c.bot.get_chat(f"@{username}")
                target_uid = chat.id
                target_username = username
                target_first_name = getattr(chat, "first_name", None) or getattr(chat, "title", None)
            except:
                await u.message.reply_text(f"لم أجد المستخدم {target_raw}. إذا غير موجود بالقاعدة استخدم الأيدي.")
                return

    if not target_uid:
        await u.message.reply_text("تعذر تحديد المستخدم المطلوب")
        return

    users = storage.data.setdefault("users", {})
    user_entry = users.get(str(target_uid), {})
    if target_username:
        user_entry["username"] = target_username
    if target_first_name:
        user_entry["first_name"] = target_first_name
    if "first_name" not in user_entry and "name" in user_entry:
        user_entry["first_name"] = user_entry.get("name")
    if "first_name" not in user_entry:
        user_entry["first_name"] = "مستخدم"
    if "created_at" not in user_entry:
        user_entry["created_at"] = datetime.now().isoformat()
    users[str(target_uid)] = user_entry

    if "scores" not in storage.leaderboard_data:
        storage.leaderboard_data["scores"] = {}
    key = f"{target_uid}_{section}_{device_type}"
    storage.leaderboard_data["scores"][key] = speed

    if "removed_from_leaderboard" in storage.leaderboard_data:
        storage.leaderboard_data["removed_from_leaderboard"].pop(str(target_uid), None)
    ...
    display_name = f"@{user_entry.get('username')}" if user_entry.get("username") else user_entry.get("first_name", "مستخدم")
    await u.message.reply_text(f"تمت الإضافة بنجاح:\n{display_name}\nالصدارة: {device_type}\nالقسم: {section}\nالسرعة: {speed:.2f} WPM")

async def apply_leaderboard_files(u: Update, c: ContextTypes.DEFAULT_TYPE, uid: int, files_dict: dict):
    """تطبيق ملفات الصدارة الجديدة المرسلة من المالك"""
    try:
        import io
        
        print(f"[LEADERBOARD] Starting to apply files for user {uid}")
        print(f"[LEADERBOARD] Files dict: {files_dict}")
        
        # تحميل الملفات من البوت
        mobile_file = await c.bot.get_file(files_dict["mobile_file_id"])
        external_file = await c.bot.get_file(files_dict["external_file_id"])
        
        print(f"[LEADERBOARD] Got file objects")
        
        # تحميل محتوى الملفات
        mobile_data = io.BytesIO()
        external_data = io.BytesIO()
        
        await mobile_file.download_to_memory(mobile_data)
        print(f"[LEADERBOARD] Downloaded mobile file")
        
        await external_file.download_to_memory(external_data)
        print(f"[LEADERBOARD] Downloaded external file")
        
        mobile_data.seek(0)
        external_data.seek(0)
        
        # قراءة JSON من الملفات
        mobile_content = json.load(mobile_data)
        external_content = json.load(external_data)
        
        print(f"[LEADERBOARD] Parsed JSON files")
        
        # التحقق من صحة الملفات
        if not isinstance(mobile_content, dict) or not isinstance(external_content, dict):
            await u.message.reply_text("صيغة الملفات غير صحيحة. تأكد من أنها ملفات JSON صحيحة")
            print(f"[LEADERBOARD] Invalid file formats")
            return
        
        # عمل backup للبيانات القديمة
        if "leaderboard_backups" not in storage.data:
            storage.data["leaderboard_backups"] = {}
        storage.data["leaderboard_backups"][str(uid)] = storage.leaderboard_data.get("scores", {})
        
        # استبدال البيانات
        new_scores = {}
        
        # إضافة بيانات الجوال
        for key, score in mobile_content.items():
            new_scores[f"{key}_جوال"] = score
        
        # إضافة بيانات الخارجي
        for key, score in external_content.items():
            new_scores[f"{key}_خارجي"] = score
        
        # تحديث الصدارة
        storage.leaderboard_data["scores"] = new_scores
        storage.save_leaderboard()
        storage.mark_dirty()
        storage.save(force=True)
        
        print(f"[LEADERBOARD] Applied new leaderboard files from {uid}")
        
        await u.message.reply_text(
            f"تم تثبيت الصدارة بنجاح!\n\n"
            f"عدد السجلات:\n"
            f"📱 الجوال: {len(mobile_content)}\n"
            f"🖥️ الخارجي: {len(external_content)}"
        )
        
    except json.JSONDecodeError as e:
        await u.message.reply_text(f"خطأ في قراءة ملف JSON: {str(e)}\n\nتأكد من أن الملفات صحيحة")
        print(f"[LEADERBOARD] JSON decode error: {e}")
    except Exception as e:
        import traceback
        await u.message.reply_text(f"حدث خطأ في تطبيق الملفات: {str(e)}")
        print(f"[LEADERBOARD] Error applying files: {e}")
        traceback.print_exc()
...
def main():
    ...
    # معالج الرسائل العادية (نصية وملفات)
